...
<div class="flex items-center justify-between mb-6">
    <p class="text-slate-500 text-sm">Total: <span class="font-bold text-slate-700">{{ count($events) }}</span> event</p>
        ...
        <table class="w-full text-sm">
            <thead class="bg-slate-50 border-b border-slate-100">
                <tr>
                    ...
                    <th class="px-5 py-4 text-left font-semibold text-slate-500">#</th>
            ...
            <tbody class="divide-y divide-slate-50">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="px-5 py-4 text-slate-400">
                        <input type="checkbox" name="ids[]" value="{{ $ev['id'] }}" class="event-checkbox rounded border-slate-300 text-blue-600 focus:ring-blue-500">
                    </td>
                    ...
                    <td class="px-5 py-4">
                        @if($ev['kategori'] === 'upcoming')
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-green-50 text-green-700">Upcoming</span>
                        @else
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-yellow-50 text-yellow-700">Highlight</span>
                        @endif
                    </td>
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-2">
                            {{-- Edit button --}}
                            <button onclick="openEdit({{ $ev['id'] }}, @js($ev['nama']), @js($ev['lokasi']), @js($ev['tanggal']), {{ $ev['harga'] }}, @js($ev['kategori']), @js($ev['urlBeli'] ?? ''), @js($ev['thumbnail'] ?? ''), @js($ev['waktu'] ?? ''), @js($ev['deskripsi'] ?? ''), @js($ev['syarat_ketentuan'] ?? ''))"
...
<div id="modalAdd" class="hidden fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form action="{{ route('admin.events.store') }}" method="POST" enctype="multipart/form-data" class="px-7 py-6 space-y-4">
            @csrf
            ...
            <div class="flex gap-3 pt-2">
                ...
                <button type="submit"
                        class="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2.5 rounded-xl font-medium transition-colors">Simpan Event</button>
            </div>
        </form>
    </div>
</div>
        ...
        <form id="editForm" method="POST" enctype="multipart/form-data" class="px-7 py-6 space-y-4">
            ...
            <div>
                ...
                <div id="edit_thumbnail_preview" class="mt-2 text-xs text-slate-500 hidden">
                    Thumbnail saat ini: <a href="#" id="edit_thumbnail_link" target="_blank" class="text-blue-600 hover:underline">Lihat Gambar</a>
                </div>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" name="waktu" id="edit_waktu" placeholder="contoh: 16.00 - 23.00 (kosongkan untuk default)"
                       class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition">
            </div>
            <div>
                <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Deskripsi Event</label>
                <textarea name="deskripsi" id="edit_deskripsi" placeholder="Deskripsi lengkap event (kosongkan untuk default)" rows="3"
            ...
            <div>
                ...
                <textarea name="syarat_ketentuan" id="edit_syarat_ketentuan" placeholder="Syarat & ketentuan event (kosongkan untuk default)" rows="3"
                          class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition"></textarea>
            </div>
            <div class="flex gap-3 pt-2">
                <button type="button" onclick="document.getElementById('modalEdit').classList.add('hidden')"
                        class="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 py-2.5 rounded-xl font-medium transition-colors">Batal</button>
                <button type="submit"
                        class="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2.5 rounded-xl font-medium transition-colors">Update Event</button>
            </div>
        </form>
    </div>
</div>
